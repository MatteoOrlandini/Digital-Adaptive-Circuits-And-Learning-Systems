...
\begin{frame}
	...
	\item formato da articoli in inglese, tedesco e olandese per
		gli utenti che non sono in grado o non vogliono leggere la versione scritta
		dell'articolo. 
...
\begin{frame}
	...
	solo ``alexkillby'' oppure si può trovare ``user:popularoutcast'', mentre in altri solo ``popularoutcast''.
\end{frame}
...
\begin{frame}
	...
	\item ``target\_words.json'' contiene al massimo 10 target words.
...
\begin{frame}
	\begin{block}{Calcolo dello spettrogramma delle parole}
	Per ogni istanza delle parole, prendiamo una finestra di mezzo secondo centrata
	sulla parola, calcoliamo lo spettrogramma mel da 128 bin e lo portiamo
	scala logaritmica.
	\end{block}
	\begin{columns}
	\column{0.5\textwidth}

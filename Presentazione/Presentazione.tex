...
\begin{frame}[fragile]
	...
	\end{block}
	\begin{lstlisting}[language=iPython,firstnumber=1, caption=Calcolo dell'accuracy, label=calcolo_accuracy,captionpos=b]
...
\begin{frame}
	Per il calcolo della loss le operazioni sono:
	\begin{enumerate}
	\item il support set e il query set vengono passati al modello ottenendo in uscita, rispettivamente, un tensore di dimensioni $(C \cdot K) \times 64 \times 15 \times 5$ e $(C \cdot Q) \times 64 \times 15 \times 5$.
	\item i tensori vengono ridimensionati in modo che la terza dimensione sia pari a 5	
	\item gli embedding di ogni classe del support set vengono sommati elemento per elemento
	\item si concatenano gli embedding del support con quelli del query set
	\item le coppie vengono date in ingresso al secondo modello che restituisce un punteggio di somiglianza
	\item viene calcolata la loss come il mean square error tra la label e il punteggio di somiglianza
	\end{enumerate}
\end{frame}
...
\begin{frame}
	...
	Per ognuna delle $C \cdot Q$ predizioni, si contano le volte in cui la j-esima predizione è uguale alla label della j-esima query. Questo valore è contenuto nella variabile \texttt{total\_rewards}. Per ogni episodio di validation, all'aumentare del numero di predizioni corrette, il valore di total rewards tende a $C \cdot Q$.
	
	\begin{equation}\label{eq:acc_relation}
	\text{mean accuracy} = \dfrac{\text{total rewards}}{C \cdot Q \cdot 1000}
	\end{equation} 
\end{frame}
...
\begin{frame}
	...
	La rete produce una coppia di valori che indicano la probabilità con cui
	una query è associata ai due set. Di questi viene presa solo l'elemento 0 che	indica l'affinità con il positive set. Utilizzando poi un corrispettivo valore che contiene la label è possibile calcolare precision, recall e AUPRC.
\end{frame}

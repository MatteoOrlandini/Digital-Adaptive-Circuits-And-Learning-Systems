...
Le concatenazioni hanno dimensione $(C\cdot Q \cdot C)\times 64 \times 5 \times 5$, infatti ogni query ($C \times Q$) va concatenata con il vettore che rappresenta ognuna delle classi (C).
...
La rete decisionale restituisce quindi un tensore \texttt{relations} $(C \cdot Q \cdot C) \times 1 \times 1$  che viene poi suddiviso in una matrice $(C \cdot Q) \times C$, in cui in ogni riga sono presenti le probabilità che una certa query appartenga ad una determinata classe.
